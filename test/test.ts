...
describe('devalue', () => {
  ...
  describe('basics', () => {
    ...
    test('RegExp', /regexp/gim, '/regexp/gim')
    ...
    test(
      'toJSON (string) Array',
      new ToJSONTest('["a", "b", "c"]'),
      '["a","b","c"]'
    )
    test('toJSON (string) Array (empty)', new ToJSONTest('[]'), '[]')
    test(
      'toJSON (string) Object',
      new ToJSONTest('{"foo":"bar","x-y":"z"}'),
      '{foo:"bar","x-y":"z"}'
    )
    test(
      'toJSON (string) (deep)',
      new ToJSONTest('{"foo":"bar","x-y": [1,2,{"a":"b"}]}'),
      '{foo:"bar","x-y":[1,2,{a:"b"}]}'
    )
  ...
  describe('cycles', () => {
    ...
    test(
      'Map (cyclical)',
      map,
      '(function(a){a.set("self", a);return a}(new Map))'
    )
    ...
    test(
      'Set (cyclical)',
      set,
      '(function(a){a.add(a).add(42);return a}(new Set))'
    )
  ...
  describe('misc', () => {
    ...
    test(
      'Object without prototype with values',
      // @ts-ignore
      Object.assign(Object.create(null), { foo: 1 }),
      'Object.create(null,{foo:{writable:true,enumerable:true,value:1}})'
    )
